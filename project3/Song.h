...
class Song{
 public:
  ...
  Song(std::string title, std::string artist, int year, int rank );
  ...
  void SetArtist(std::string artist); //sets artist
  ...
  friend std::ostream &operator<< (std::ostream &output, Song &mySong) {
    output << "\"" << mySong.m_title << "\" by " << mySong.m_artist << " "
	   << mySong.m_rank << "/" << "100 from " << mySong.m_year;
    return output;
  }
 private:
  std::string m_title; //Title of song
  std::string m_artist; //Artist of song
